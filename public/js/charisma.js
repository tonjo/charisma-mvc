$(document).ready(function () {
    ...
    $('#themes a').click(function (e) {
        ...
        $.cookie('currentTheme', currentTheme, {path: '/', expires: 365});
        switchTheme(currentTheme);
    });


    function switchTheme(themeName) {
        var str = $('#bs-css').attr('href');
        var ind = str.lastIndexOf('/');
        if (ind >= 0) {
            if (themeName == 'classic') {
                myself = document.getElementById('charisma.js');
                arr = myself.src.split('/',4);
                strnew = arr.join('/') + '/bower_components/bootstrap/dist/css/bootstrap.min.css';
            }
            else {
                strnew = str.substring(0,ind+1) + "bootstrap-" + themeName + ".min.css";
            }
            $('#bs-css').attr('href', strnew);
        }
...
function docReady() {
    ...
    $('#file_upload').uploadify({
        'swf': 'application/libs/uploadify.swf',
        'uploader': 'application/libs/uploadify.php'
